...
urlpatterns = [
    ...
    path(
        "<uuid:campaign_id>/participation/",
        views.CampaignParticipationView.as_view(),
        name="join-or-leave-campaign",
    ),
    path("user/", views.UserDetailsAPIView.as_view(), name="fetch-user"),
    path(
        "edit/<uuid:campaign_id>/",
        views.UpdateCampaignStatusAPIView.as_view(),
        name="update-camp-status",
    ),
    path(
        "user-campaigns/", views.UserCampaignsAPIView.as_view(), name="user-campaigns"
    ),
]
