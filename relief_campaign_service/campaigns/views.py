...
from .serializers import CampaignParticipationSerializer, CampaignSerializer
...
class CampaignParticipationView(APIView):
    ...
    #join
        if action == "join":
            ...
            volunteer_count = campaign.participations.filter(
                participant_type=CampaignParticipation.VOLUNTEER
            ).count()

            if participation.participant_type == CampaignParticipation.VOLUNTEER:
                campaign.volunteers_registered = max(0, volunteer_count - 1)
...
class UserDetailsAPIView(APIView):
    ...
    def get(self, request):
        logger.info(
            f"Received Authorization header: {request.headers.get('Authorization')}"
        )
        user = request.user  # Populated from RabbitMQ-based authentication
...
class UpdateCampaignStatusAPIView(APIView):
    permission_classes = [IsAuthenticated]
